            ...
            Layout::table('table', [
                TD::make('edit', '')
                    ...
                    ->render(fn (Repository $model) =>
                        '<a href="' . route('platform.product.edit', $model->get('product_id')) . '" class="btn btn-warning center-text">Edit</a>'
                    ),

                TD::make('product_id', 'ID produs')
                    ->width('50')
                    ->render(fn (Repository $model) => $model->get('product_id'))
                    ->sort(),
                ...
                TD::make('brand', 'Brand')
                    ->width('100')
                    ->render(fn (Repository $model) => ucfirst($model->get('brand')))
                    ->sort(),

                TD::make('model', 'Model')
                    ->width('100')
                    ->render(fn (Repository $model) => ucfirst($model->get('model')))
                    ->sort(),
                ...
                TD::make('specifications', 'Specificații')
                    ->width('200')
                    ->render(fn (Repository $model) => Str::limit($model->get('specifications'), 100)),

                TD::make('entered_at', 'Intrat la')
                    ->width('200')
